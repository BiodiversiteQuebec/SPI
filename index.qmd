---
title: "Indicateur de protection des espèces"
format:
  html:
    theme: flatly
    toc: true
    code-fold: show
    code-tools: true
    source: repo
---
```{r, include = F}
# packages
library(knitr)
library(mapview)
library(plotly)
library(sf)
library(terra)
library(htmltools)
library(viridis)
library(smoothr)
library(rmapshaper)
library(leaflet)

# raw data
aires <- st_read("data_clean/aires_protegees_simplifiees.gpkg")
# sum(aires$HA_LEGAL) # superficie totale des aires protegees = 28060703 ha
aires_latlon <- st_transform(aires, crs = st_crs(4326))
aires_latlon$HA_LEGAL2 <- round(aires_latlon$HA_LEGAL, digits = 1)
aires_latlon$POPINFOS <- paste0("<b>", aires_latlon$NOM, "</b><br> date de création: <b>", aires_latlon$DA_CREATIO, "</b><br> superficie: <b>", aires_latlon$HA_LEGAL2, " ha </b>")
SPI <- read.csv("results/SPI.csv")[, -1]
names <- as.character(unique(SPI$SPECIES))
years <- as.numeric(unique(SPI$YEAR))

SPI_OCC <- read.csv("results/SPI_occ.csv") # ! pas les meme especes que dans SPI
names_occ <- as.character(unique(SPI_OCC$SPECIES))
# data treatment - remove is.na(SPI_OCC) concerning "Moerckia blyttii" &  "Information masquée"
SPI_OCC <- SPI_OCC[!is.na(SPI_OCC$SPI), ]
SPI_OCC <- SPI_OCC[!(SPI_OCC$SPECIES == "Information masquée"), ]


# produced data
# -----------------------
# --- SPI - distribution
# ------------------------

all_sm <- data.frame()
for (i in names) {
    m <- cbind(SPI$YEAR[SPI$SPECIES == i], SPI$SPI[SPI$SPECIES == i])
    sm <- smooth_ksmooth(m, smoothness = 4)
    df <- as.data.frame(sm)
    names(df) <- c("YEAR", "SPI")
    df$SPECIES <- i
    df$YEAR2 <- floor(df$YEAR)
    df$SPI2 <- round(df$SPI, digits = 3)
    df$INFOS <- paste0("<b>", df$SPECIES, "</b><br>Année <b>", df$YEAR2, "</b><br>SPI = <b>", df$SPI2, "</b>")
    all_sm <- rbind(all_sm, df)
}

# mean SPI
# --------
year_mean <- c()
for (i in years) {
    sub_year <- SPI$SPI[SPI$YEAR == i]
    year_mean <- c(year_mean, mean(sub_year, na.rm = TRUE))
}

# smooth mean & min-max values
# -----------------------------
mm <- cbind(years, year_mean)
s_mm <- smooth_ksmooth(mm, smoothness = 4)
df_max <- SPI[SPI$SPECIES == "Pseudacris maculata", ]
df_min <- SPI[SPI$SPECIES == "Desmognathus ochrophaeus", ]
s_max <- smooth_ksmooth(as.matrix(df_max[, c(3, 2)]), smoothness = 4)
s_min <- smooth_ksmooth(as.matrix(df_min[, c(3, 2)]), smoothness = 4)

s_range <- list(s_mm, s_max, s_min)
names(s_range) <- c("s_mm", "s_max", "s_min")

s_range <- lapply(s_range, function(x) {
    df <- as.data.frame(x)
    names(df) <- c("YEAR_PRED", "SPI")
    df$SPI2 <- round(df$SPI, digits = 3)
    df$YEAR2 <- floor(df$YEAR)
    df
})

s_mm <- s_range[["s_mm"]]
s_mm$SP <- "Valeur moyenne"
s_max <- s_range[["s_max"]]
s_max$SP <- "Pseudacris maculata"
s_min <- s_range[["s_min"]]
s_min$SP <- "Desmognathus ochrophaeus"

sm_ls <- list(s_mm, s_min, s_max)
names(sm_ls) <- c("s_mm", "s_min", "s_max")

sm_ls <- lapply(sm_ls, function(x) {
    x$INFOS <- paste0("<b>", x$SP, "</b><br>Année <b>", x$YEAR2, "</b><br>SPI = <b>", x$SPI2, "</b>")
    x
})

s_mm <- sm_ls[[1]]
s_min <- sm_ls[[2]]
s_max <- sm_ls[[3]]

# ---------------------
# --- SPI - occurrence
# ----------------------

all_sm_occ <- data.frame()
spe_name <- unique(SPI_OCC$SPECIES)
for (i in spe_name) {
    m <- cbind(SPI_OCC$YEAR[SPI_OCC$SPECIES == i], SPI_OCC$SPI[SPI_OCC$SPECIES == i])
    sm <- smooth_ksmooth(m, smoothness = 4)
    df <- as.data.frame(sm)
    names(df) <- c("YEAR", "SPI")
    df$SPECIES <- i
    df$YEAR2 <- floor(df$YEAR)
    df$SPI2 <- round(df$SPI, digits = 3)
    df$INFOS <- paste0("<b>", df$SPECIES, "</b><br>Année <b>", df$YEAR2, "</b><br>SPI = <b>", df$SPI2, "</b>")
    all_sm_occ <- rbind(all_sm_occ, df)
}

# mean SPI
# --------
year_mean_occ <- c()
for (i in years) {
    sub_year <- SPI_OCC$SPI[SPI_OCC$YEAR == i]
    year_mean_occ <- c(year_mean_occ, mean(sub_year, na.rm = TRUE))
}

# smooth mean & min-max values
# ----------------------------

# mean
mm_occ <- cbind(years, year_mean_occ)
s_mm_occ <- smooth_ksmooth(mm_occ, smoothness = 4)
s_mm_occ <- as.data.frame(s_mm_occ)
names(s_mm_occ) <- c("YEAR_PRED", "SPI")
s_mm_occ$SPECIES <- "Valeur moyenne"

# min - max
SPI_OCC2023 <- SPI_OCC[SPI_OCC$YEAR == 2023, ]
summary(SPI_OCC2023)

max_sp <- unique(SPI_OCC2023$SPECIES[SPI_OCC2023$SPI == max(SPI_OCC2023$SPI)]) # only one species - "Forsstroemia trichomitria"
min_sp <- unique(SPI_OCC2023$SPECIES[SPI_OCC2023$SPI == min(SPI_OCC2023$SPI)]) # 100 species with SPI = 0

df_max_occ <- SPI_OCC[SPI_OCC$SPECIES %in% max_sp, ]
s_max_occ <- smooth_ksmooth(as.matrix(df_max_occ[, c(3, 2)]), smoothness = 4)
s_max_occ <- as.data.frame(s_max_occ)
names(s_max_occ) <- c("YEAR_PRED", "SPI")
s_max_occ$SPECIES <- max_sp

df_min_occ <- SPI_OCC[SPI_OCC$SPECIES %in% min_sp, ]
df_ls <- split(df_min_occ, df_min_occ$SPECIES)
df_ls2 <- lapply(df_ls, function(x) {
    mat <- smooth_ksmooth(as.matrix(x[, c(3, 2)]), smoothness = 4)
    mat_df <- as.data.frame(mat)
    names(mat_df) <- c("YEAR_PRED", "SPI")
    mat_df$SPECIES <- unique(x$SPECIES)
    mat_df
})
s_min_occ <- do.call("rbind", df_ls2)

# global DF
s_occ_range <- rbind(s_mm_occ, s_max_occ, s_min_occ)
s_occ_range$INFOS <- paste0("<b>", s_occ_range$SPECIES, "</b><br>Année <b>", floor(s_occ_range$YEAR), "</b><br>SPI = <b>", round(s_occ_range$SPI, digits = 3), "</b>")

# ----------------------------
# --- last SPI - distribution
# ----------------------------

last_spi <- read.csv2("data_clean/SPI_north_south.csv")

# Output figure in HTML5
# if (knitr::is_html_output()) {
#     knitr::knit_hooks$set(
#         plot = function(x, options) {
#             cap <- options$fig.cap # figure caption
#             tags <- htmltools::tags
#             as.character(tags$figure(
#                 tags$img(src = x, alt = cap),
#                 tags$figcaption(cap)
#             ))
#         }
#     )
# }
```

# Points clé

## **Représentation actuelle des aires protégées du Québec**

Superficie totale protégée = 28 060 703 ha

```{r, echo=F}
#| label: fig-aires_prot
#| fig-cap: "Carte des aires protégées actuelles."
#| warning: false
# aires_latlon$year <- as.factor(aires_latlon$year)
# mapview(aires,
#         # col.regions = "#238A8DFF",
#         popup = T,
#         legend = F,
#         zcol = "year"
#         # , burst = T
#         )
# col_pal <- viridis(4, direction = -1)
col_pal <- c("#440154FF", "#39568CFF", "#1F968BFF", "#73D055FF")
m <- leaflet(aires_latlon) |>
    addTiles() |>
    addPolygons(
        data = aires_latlon[aires_latlon$year < 1900, ],
        color = col_pal[1],
        weight = 1,
        smoothFactor = 0.5,
        opacity = 1.0,
        fillOpacity = 0.5,
        group = "Antérieure à 1900",
        highlightOptions = highlightOptions(
            color = "white",
            weight = 2,
            bringToFront = TRUE
        ),
        popup = aires_latlon$POPINFOS[aires_latlon$year < 1900]
    ) |>
    addPolygons(
        data = aires_latlon[aires_latlon$year >= 1900 & aires_latlon$year < 1950, ],
        color = col_pal[4],
        weight = 1,
        smoothFactor = 0.5,
        opacity = 1.0,
        fillOpacity = 0.5,
        group = "Période 1900 - 1950",
        highlightOptions = highlightOptions(
            color = "white",
            weight = 2,
            bringToFront = TRUE
        ),
        popup = aires_latlon$POPINFOS[aires_latlon$year >= 1900 & aires_latlon$year < 1950]
    ) |>
    addPolygons(
        data = aires_latlon[aires_latlon$year >= 1950 & aires_latlon$year < 2000, ],
        color = col_pal[2],
        weight = 1,
        smoothFactor = 0.5,
        opacity = 1.0,
        fillOpacity = 0.5,
        group = "Période 1950 - 2000",
        highlightOptions = highlightOptions(
            color = "white",
            weight = 2,
            bringToFront = TRUE
        ),
        popup = aires_latlon$POPINFOS[aires_latlon$year >= 1950 & aires_latlon$year < 2000]
    ) |>
    addPolygons(
        data = aires_latlon[aires_latlon$year >= 2000, ],
        color = col_pal[3],
        weight = 1,
        smoothFactor = 0.5,
        opacity = 1.0,
        fillOpacity = 0.5,
        group = "Postérieure à 2000",
        highlightOptions = highlightOptions(
            color = "white",
            weight = 2,
            bringToFront = TRUE
        ),
        popup = aires_latlon$POPINFOS[aires_latlon$year >= 2000]
    ) |>
    addLayersControl(
        overlayGroups = c(
            "Antérieure à 1900",
            "Période 1900 - 1950",
            "Période 1950 - 2000",
            "Postérieure à 2000"
        ),
        options = layersControlOptions(collapsed = FALSE)
    )
m
```

## **Augmentation de la protection de toutes les espèces ciblées au cours des 61 dernières années**  
Augmentation minimale de 1% pour la Salamande sombre des montagnes (*Desmognathus ochrophaeus*)  
Augmentation maximale de 70% pour la Rainette faux grillon (*Pseudacris maculata*)  
Avec une moyenne de 11% pour les 225 espèces considérées  
À mettre en lien avec le seuil Aichi de 17% et/ou les cibles du cadre mondial de la biodiversité 

```{r, echo=F, warnings=F}
#| label: fig-spi_trend
#| fig-cap: "Valeur moyenne et par espèce de l'indice de protection des espèces. La courbe bleue représente le SPI moyen, les courbes verte et pourpre représentent les espèces la moins et la plus protégée, respectivement."
#| warning: false

g <- plot_ly() |> add_lines(
    x = s_mm$YEAR_PRED,
    y = s_mm$SPI,
    color = I("#238A8DFF"),
    text = s_mm$INFOS,
    hoverinfo = "text"
)

for (i in names) {
    sm <- all_sm[all_sm$SPECIES == i, ]
    g <- add_lines(g,
        x = sm$YEAR,
        y = sm$SPI,
        color = I("lightgrey"),
        text = sm$INFOS,
        hoverinfo = "text"
    )
}
g <- add_lines(g, x = s_mm$YEAR_PRED, y = s_mm$SPI, color = I("#238A8DFF"), text = s_mm$INFOS, hoverinfo = "text")
g <- add_lines(g, x = s_min$YEAR_PRED, y = s_min$SPI, color = I("#B8DE29FF"), text = s_min$INFOS, hoverinfo = "text")
g <- add_lines(g, x = s_max$YEAR_PRED, y = s_max$SPI, color = I("#404788FF"), text = s_max$INFOS, hoverinfo = "text")
g |> layout(
    showlegend = F,
    yaxis = list(zeroline = FALSE, title = "SPI"),
    xaxis = list(zeroline = FALSE, title = "Année")
)
```

```{r, echo=F, warnings=F}
#| label: fig-spi_occ_trend
#| fig-cap: "Valeur moyenne et par espèce de l'indice de protection des espèces. La courbe bleue représente le SPI moyen, les courbes verte et pourpre représentent les espèces la moins et la plus protégée, respectivement."
#| warning: false

g <- plot_ly() |> add_lines(
    x = s_mm_occ$YEAR_PRED,
    y = s_mm_occ$SPI,
    color = I("#238A8DFF"),
    text = s_mm_occ$INFOS,
    hoverinfo = "text"
)

for (i in spe_name) {
    sm <- all_sm_occ[all_sm_occ$SPECIES == i, ]
    g <- add_lines(g,
        x = sm$YEAR,
        y = sm$SPI,
        color = I("lightgrey"),
        text = sm$INFOS,
        hoverinfo = "text"
    )
}
g <- add_lines(
    g,
    x = s_mm_occ$YEAR_PRED,
    y = s_mm_occ$SPI,
    color = I("#238A8DFF"),
    text = s_mm_occ$INFOS,
    hoverinfo = "text")
g <- add_lines(
    g,
    x = s_max_occ$YEAR_PRED,
    y = s_max_occ$SPI,
    color = I("#404788FF"),
    text = s_max_occ$INFOS,
    hoverinfo = "text")

for(i in unique(s_min_occ$SPECIES)) {
        g <- add_lines(
            g,
            x = s_min_occ$YEAR_PRED[s_min_occ$SPECIES == i],
            y = s_min_occ$SPI[s_min_occ$SPECIES == i],
            color = I("#B8DE29FF"),
            text = s_min_occ$INFOS[s_min_occ$SPECIES == i],
            hoverinfo = "text")

}

g |> layout(
    showlegend = F,
    yaxis = list(zeroline = FALSE, title = "SPI"),
    xaxis = list(zeroline = FALSE, title = "Année")
)
```

## **Moments marquants de la création des aires protégées**  
Regarder autour des années 1981 et 2003 qui correspondent à une augmentation importante de la superficie des aires protégées.  

## **Etat de la protection des espèces du nord vs. du Sud**  
*Une espèce est considérée du nord si la latitude du centroïde de son aire de répartition est supérieure à 55°. Inversement, une espèce dont la latitude du centroïde de son aire de répartition est inférieure ou égale à cette valeur seuil, sera considérée comme une espèce du sud.*  
Les espèces du Nord présentent un SPI moyen de 17% (sd = 4%).  
Les espèces du Sud présentent un SPI moyen 9% (sd = 8%).  

```{r, echo=F, warning = FALSE}
#| label: fig-spi_2023
#| fig-cap: "Valeur de l'indicateur de protection des espèces en 2023."
#| warning: false

# conversion from species characters to ordered levels of factor for keeping increasing SPI
last_spi$SPECIES2 <- factor(last_spi$SPECIES, levels = last_spi$SPECIES[order(last_spi$SPI, decreasing = FALSE)])

# fonction for drawing a vertical line
vline <- function(x = 0, color = "#238A8DFF") {
    list(
        type = "line",
        y0 = 0,
        y1 = 1,
        yref = "paper",
        x0 = x,
        x1 = x,
        line = list(color = color, dash = "dot")
    )
}

fig <- plot_ly() |>
    add_bars(
        data = last_spi,
        x = ~SPI,
        y = ~SPECIES2,
        type = "bar",
        orientation = "h",
        color = ~LOC,
        # colors = pal2,
        hovertemplate = paste(
            "<b>%{y}</b><br>",
            "<b>SPI</b> = %{x}"
        )
    ) |>
    layout(
        yaxis = list(zeroline = FALSE, title = "Espèce", showticklabels = FALSE),
        xaxis = list(zeroline = FALSE),
        shapes = list(vline(0.17, color = "black"))
    )
# print(fig)
fig
```


## **Identification d'espèces extrêmes en terme de protection**  
cf la Salamandre sombre des montagnes et la Rainette faux grillon

